...
struct VaccinationController: Controller, AdminUpdateController, ImageUploadController {
	func createNewModel(req: Vapor.Request) async throws -> VaccinationData {
		let model = VaccinationData(status: .humanReviewRequired)
		try await model.save(on: req.db)
		return model
	}

	func getExistingModel(for user: User, on db: FluentKit.Database) async throws -> VaccinationData? {
		return try await user.$vaccinationData.get(on: db)
	}

	func adminUpdate(with update: AdminUpdate, for user: User, on db: FluentKit.Database) async throws -> VaccinationData {
		guard let model = try await getExistingModel(for: user, on: db) else {
			throw Abort(.notFound, reason: "No vaccination record exists for that user.")
		}
	...
	typealias ObjectType = VaccinationData
	typealias AdminUpdate = StatusUpdate

	func boot(routes: RoutesBuilder) throws {
		let vaccinations = routes.grouped("vaccinations")
		let authed = vaccinations.grouped([AccessToken.authenticator(), AccessToken.guardMiddleware()])

		try allRoutes(routes: vaccinations)

		authed.post("verified", use: uploadVerified)
	}
	...
	func uploadVerified(req: Request) async throws -> VaccinationData.Response {
		...
		let vaccineData: VaccinationData = try await user.$vaccinationData.get(on: req.db) ?? VaccinationData()
		let status: VerificationStatus

		if
			(
				firstName.lowercased() == patientFirstName.lowercased() ||
				req.healthCardConfig.nicknames.value[firstName.lowercased()]?.contains(where: { $0 == patientFirstName.lowercased() }) == true
			) && lastName.lowercased() == patientLastName.lowercased()
		{
			vaccineData.status = .verified
			req.logger.log(level: .debug, "Names matched")
		} else {
			vaccineData.status = .verifiedWithDiscrepancy
		}

		try await vaccineData.update(with: .verified(record: record), on: req.db)

		try await vaccineData.save(on: req.db)

		return try await vaccineData.getResponse(on: req.db)
...
//		}
//
//		let response = VaccinationData.Response(status: user.vaccinationData.sta, record: vaccinationRecord, lastUpdated: Date())
//
